...
import Chat from './ChatAliceChatItem';
import UserItem from './ChatAliceChatUserItem';
...
const ChatAliceChat = ({
    ...
    const { mode, language, botmode } = formData;
    ...
    useEffect(() => {
        ...
        if (!club) getClub(match.params.id);
    ...
    const onChangeFormData = (e) => {
        if (user._id === club.classmonitor._id) {
            ...
            modeUpdate(userData, match.params.id, socket);
        } else {
            setMsg('방장만 변경 가능합니다!!!');
            handleOpen();
        }
    ...
    const onSubmit = (data, e) => {
        ...
        if (data.message !== '') {
            addClubChat(socket, data.message, match.params.id);
            reset({ message: '' });
        }
    };
                    ...
                    <Grid item xs={2} className={classes.borderRight500}>
                        ...
                        <Grid item xs={12} style={{ padding: '10px' }}>
                            ...
                            <FormControl component="fieldset">
                                ...
                                <RadioGroup aria-label="botmode" name="botmode" value={botmode} onChange={(e) => onChangeFormData(e)}>
                                    {language === 'en' ? (
                                        <>
                                            <FormControlLabel value="scenario" control={<Radio />} label="scenario" />
                                            <FormControlLabel value="travel" control={<Radio />} label="travel" />
                                            <FormControlLabel value="intent" control={<Radio />} label="intent" />
                                        </>
                                    ) : (
                                            <>
                                                {' '}
                                                <FormControlLabel value="scenario" control={<Radio />} label="scenario" />
                                                <FormControlLabel value="gag" control={<Radio />} label="gag" />
                        ...
                        <List>
                            <ListItem>
                                <ListItemIcon>
                                    <Avatar className={classes.red} alt="Alice" src="/broken-image.jpg" />
                                </ListItemIcon>
                        ...
                        <List className={classes.messageArea}>
                            ...
                            {chats.length > 0 &&
                                chats.map((chat, i, arr) =>
                                    i === arr.length - 1 ? (
                                        <Chat key={chat._id} chat={chat} auth={user}></Chat>
                                    ) : (
                                            <Chat key={chat._id} last={false} chat={chat} auth={user}></Chat>
                                        )
                                )}
...
export default connect(mapStateToProps, {
    ...
    modeUpdate,
    modeChangeSocket,
})(SpeechRecognition(options)(withRouter(ChatAliceChat)));
