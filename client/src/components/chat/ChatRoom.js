...
import { personalGet, personalChat, personalClear } from '../../actions/personal';
...
const ChatRoom = ({
    ...
    const [httpCheck, setHttpCheck] = useState(false);
    ...
    const onClickMessage = () => {
        if (message !== '') {
            stopListening();
            personalChat(message, match.params.id);
            // setTtsCheck(false);
            setMessage(' ');
    ...
    recognition.onresult = (event) => {
        ...
        } else if (stopCmd[0] === '홈페이지' && stopCmd[1] === '고마워') {
            windowCheck.close();
        }
    };

    return loading || personal === null ? (
        <Spinner />
    ) : (
            <>
                <Grid container>
                                ...
                                personal.map((chat, i, arr) =>
                                    i === arr.length - 1 ? (
                                        <Chat
                                            ...
                                            setHttpCheck={setHttpCheck}
                                            stopListening={stopListening}
                                            startListening={startListening}
                                            micCheck={micCheck}
                                            setWindowCheck={setWindowCheck}
                                            chat={chat}
                                        ></Chat>
                                    ) : (
...
export default connect(mapStateToProps, { personalGet, personalChat, personalClear })(SpeechRecognition(options)(withRouter(ChatRoom)));
